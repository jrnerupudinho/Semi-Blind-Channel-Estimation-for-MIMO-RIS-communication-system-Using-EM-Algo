...
def channelMatrix(n_tx,n_rx,N,varh):
  ...
  h = khatri_rao(H_BS.T, H_SU).flatten()
  return h
  ...
  for l in range(itera):
    ...
    for t in range(0,T_d):
      ...
      for k in range(0,np.power(M,n_tx)):
        ...
        beta_denominator += mp.exp(-np.power(norm(first_dummy_den),2)/np.power(varn,2))*(-10**(-300))
      for j in range(0,np.power(M,n_tx)):
        ...
        beta_exp = mp.exp(-np.power(norm(first_dummy),2)/(varn**2))/beta_denominator*(-10**(-300))
        # print(float(beta_exp)*np.matmul(np.conjugate(np.kron(np.kron(PsiTilde_td[:,t][np.newaxis],all_possibleSymbols[j][np.newaxis]),np.eye(n_rx,dtype='complex128'))).T,Y_d[t]))
...
def noiseSignals_proposed(varn,n_rx,T_d,T_p):
    N_p = np.random.normal(loc=0, scale=np.sqrt(varn/2), size=(n_rx, T_p*2)).view(np.complex128)
    N_d = np.random.normal(loc=0, scale=np.sqrt(varn/2), size=(n_rx,T_d*2)).view(np.complex128)
    return N_p,N_d
    
def receivedSignals(T_p,T_d,PsiTilde_tp,PsiTilde_td ,n_rx,n_tx,X_d,X_p,h,varn,M_symbols,N_p,N_d):
  Z_p = []
  Z_d = []
  Y_p = []
  Y_d = []
  for i in range(T_p):
    ...
    Y_p.append(np.matmul(Z_pt,h[:,np.newaxis]) + N_p[:,i][:,np.newaxis]) #creates a list with arrays
  for j in range(T_d):
    ...
    Y_d.append(np.matmul(Z_dt,h[:,np.newaxis]) + N_d[:,j][:,np.newaxis])
...
def main():
    ...
    for i in range(monte_iter):
        ...
        for tp in range(len(T_p)):      
          PsiTilde_tp,PsiTilde_td = irsMatrix(T_p[tp],T_d,N)
          ...
          N_p,N_d = noiseSignals_proposed(varn,n_rx,T_d,T_p[tp])
          # X_d,all_possibleSymbols = symbols(T_p[tp],T_d,n_tx,M_symbols)
          Y_p,Y_d,Z_p,Z_d = receivedSignals(T_p[tp],T_d,PsiTilde_tp,PsiTilde_td ,n_rx,n_tx,X_d,X_p,h,varn,M_symbols, N_p,N_d )
