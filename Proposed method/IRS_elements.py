...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 13 15:23:42 2023
...
def em_mmse(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,h,N):
  n_rx,_ = Y_d[0].shape
  theta_t = h_initial
...
def em_zf(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,h,N):
  n_rx,_ = Y_d[0].shape
  theta_t =h_initial
  print("inital theta",norm(theta_t))
  # e_firstTerm =  T_d*n_tx*np.log(M)
...
def em_ml(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,N):
  n_rx,_ = Y_d[0].shape
...
def em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td,M,varn,itera,h_initial,h,n_tx,partition_r,X_d,qamCons,N):
  n_rx,_ = Y_d[0].shape
  theta_t = h_initial
  print("inital theta",norm(theta_t))
  m = np.log2(M)
...
def em(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,N):
  n_rx,_ = Y_d[0].shape
  theta_t = h_initial
...
T_d = 45; # Number of pilot symbols
# T_p = [4]
N = [10,20,30,40,50,60] ; # Number of IRS elements
...
partition_r = 1
# #Computation 
mse_ml = np.zeros((monte_iter,len(N)))
mse_ZF = np.zeros((monte_iter,len(N)))
mse_mmse = np.zeros((monte_iter,len(N)))
# mse_ecu = np.zeros((monte_iter,len(T_p)))
mse_ecu_2 = np.zeros((monte_iter,len(N)))
mse = np.zeros((monte_iter,len(N)))
for i in range(monte_iter):
    ...
    for n in range(len(N)):
      h = channelMatrix(n_tx,n_rx,N[n],varh)
     
      PsiTilde_tp,PsiTilde_td = irsMatrix(T_p,T_d,N[n],beta_min,amp)
      ...
      Y_p,Y_d,Z_p,Z_d,h_initial = receivedSignals(T_p,T_d,PsiTilde_tp,PsiTilde_td ,n_rx,n_tx,X_d,X_p,h,varn,M_symbols)
      ...
      # theta_hat_ec2 = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,M_symbols,varn,itera,h_initial,h,n_tx,partition_r,X_d,qamCons,N[n])
      # theta_hat_ml = em_ml(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,N[n])
      # theta_hat_ZF = em_zf(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h,N[n])
      # theta_hat_mmse = em_mmse(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h,N[n])
      theta_hat = em(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,N[n])
      print("original:",norm(h))
      # print("predicted",norm(theta_hat_ec))
      # mse_ecu_2[i][n] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ec2-h[:,np.newaxis]).T),(theta_hat_ec2-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      ...
      # mse_ml[i][n] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ml-h[:,np.newaxis]).T),(theta_hat_ml-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      # mse_ZF[i][n] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ZF-h[:,np.newaxis]).T),(theta_hat_ZF-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      # mse_mmse[i][n] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_mmse-h[:,np.newaxis]).T),(theta_hat_mmse-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse[i][n] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat-h[:,np.newaxis]).T),(theta_hat-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
...
plt.plot(N, mse_ecu_2, label = 'Soft decision-PM ',linestyle=':')
plt.plot(N, mse_ml, label = 'Non - Superimposed log-max',linestyle='dashed', marker='v',)
plt.plot(N, mse_ZF, label = 'Zero forcing',linestyle='-.')
plt.plot(N, mse_mmse, label = 'MMSE', linestyle='dashed', marker='o',)
plt.plot(N, mse, label = 'Non - Superimposed',linestyle='dashed', marker='s')
