...
def em_zf(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,h):
  ...
  for l in range(itera):
    ...
    for t in range(0,T_d):
      ...
      # data_est_approx = np.linalg.pinv(channel)@data_est_approx
      # beta_denominator = 0  
...
for i in range(monte_iter):
    ...
    for tp in range(len(T_p)):      
      ...
      theta_hat_ec,logLikelihood = em_zf(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h)
...
plt.plot(T_p, mse_ecu, label = 'ZF-Euclidean')
plt.grid(color = 'red', linestyle = '--', linewidth = 0.5)
plt.ylabel('NMSE')
plt.xlabel('T_p')
plt.xticks(T_p)
plt.yscale("log")
plt.title(' Proposed method ZF')
plt.legend(loc='best')
plt.show()