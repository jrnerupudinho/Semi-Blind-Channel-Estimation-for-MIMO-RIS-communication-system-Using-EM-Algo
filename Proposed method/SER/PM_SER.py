...
T_p = 15 # Number of pilot symbols
# T_p = [4]
N = 20; # Number of IRS elements
...
# #Computation 
mse_ecu1 = np.zeros((monte_iter,len(SNR)))
mse_ecu2 = np.zeros((monte_iter,len(SNR)))
mse_ecu3 = np.zeros((monte_iter,len(SNR)))
for i in range(monte_iter):
    ...
    for k in range(len(SNR)):      
      ...
      theta_hat_ec1,X_dEstimated_1 = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h,n_tx,partition_r_1,X_d,qamCons)
      theta_hat_ec2,X_dEstimated_2 = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h,n_tx,partition_r_2,X_d,qamCons)
      # theta_hat_ec3,X_dEstimated_3 = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h,n_tx,partition_r_3,X_d,qamCons)
      print("original:",norm(h))
      print("predicted",norm(theta_hat_ec1))
      mse_ecu1[i][k] = np.count_nonzero(np.array(X_d)-np.array(X_dEstimated_1))/ (T_d*n_tx)
      mse_ecu2[i][k] = np.count_nonzero(np.array(X_d)-np.array(X_dEstimated_2))/ (T_d*n_tx)
      # mse_ecu3[i][k] = np.count_nonzero(np.array(X_d)-np.array(X_dEstimated_3))/ (T_d*n_tx)
...
mse_ecu1 = np.average(mse_ecu1, axis=0)
mse_ecu2 = np.average(mse_ecu2, axis=0)
mse_ecu3 = np.average(mse_ecu3, axis=0)
plt.plot(SNR, mse_ecu1, label = 'Soft decision-PM r = 0',linestyle='-.')
plt.plot(SNR, mse_ecu2, label = 'Soft decision-PM r = 1',linestyle=':')
plt.plot(SNR, mse_ecu3, label = 'Soft decision-PM r = 2',linestyle='--')
