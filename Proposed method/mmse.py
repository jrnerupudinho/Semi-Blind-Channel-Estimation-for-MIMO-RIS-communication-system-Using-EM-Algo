...
def em(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,h):
  n_rx,_ = Y_d[0].shape
  theta_t = h_initial
...
for i in range(monte_iter):
    ...
    for tp in range(len(T_p)):      
      ...
      theta_hat_ec,logLikelihood = em(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h)
...
plt.plot(T_p, mse_ecu, label = 'MMSE-Euclidean')
plt.grid(color = 'red', linestyle = '--', linewidth = 0.5)
plt.ylabel('NMSE')
plt.xlabel('T_p')
plt.xticks(T_p)
plt.yscale("log")
plt.title(' Proposed method MMSE')
plt.legend(loc='best')
plt.show()