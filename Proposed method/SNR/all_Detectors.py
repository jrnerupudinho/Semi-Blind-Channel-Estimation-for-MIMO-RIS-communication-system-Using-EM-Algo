...
def em_zf(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M,varn,itera,h_initial,h):
  ...
  for l in range(itera):
    ...
    theta_tPlusOne = np.linalg.solve(m_firstTermDenom + m_secondTermDenom,m_firstTermNumer + m_secondTermNumer)
    theta_t = theta_tPlusOne
    # if np.abs(norm(theta_t)-norm(h))<1:
    #     print("iterations",l)
    #     break
    print(norm(theta_t))
  # print(logLikelihood)
...
monte_iter = 15;  # Number of Monte carlo iterations
...
# #Computation 
mse_ml = np.zeros((monte_iter,len(SNR)))
mse_ZF = np.zeros((monte_iter,len(SNR)))
mse_mmse = np.zeros((monte_iter,len(SNR)))
mse_ecu = np.zeros((monte_iter,len(SNR)))
mse = np.zeros((monte_iter,len(SNR)))
# mse_ecu_2 = np.zeros((monte_iter,len(SNR)))
for i in range(monte_iter):
    ...
    for k in range(len(SNR)):      
      ...
      print(norm(Z_p))
      theta_hat_ec = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,M_symbols,varn[k],itera,h_initial,h,n_tx,partition_r_1,X_d,qamCons)
      # theta_hat_ec2 = em_pm(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h,n_tx,partition_r_2,X_d,qamCons)
      theta_hat_ml = em_ml(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial)
      theta_hat_ZF = em_zf(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h)
      theta_hat_mmse = em_mmse(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial,h)
      theta_hat = em(Y_d,Y_p,T_d,T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn[k],itera,h_initial)

      print("original:",norm(h))
      print("predicted",norm(theta_hat_ec))
      # mse_ecu_2[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ec2-h[:,np.newaxis]).T),(theta_hat_ec2-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_ecu[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ec-h[:,np.newaxis]).T),(theta_hat_ec-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_ml[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ml-h[:,np.newaxis]).T),(theta_hat_ml-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_ZF[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ZF-h[:,np.newaxis]).T),(theta_hat_ZF-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_mmse[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_mmse-h[:,np.newaxis]).T),(theta_hat_mmse-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      # mse[i][tp] = np.power(norm(np.subtract(theta_hat,h[:,np.newaxis])),2)/np.power(norm(h[:,np.newaxis]),2)
      mse[i][k] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat-h[:,np.newaxis]).T),(theta_hat-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
    if(i%5 == 0):
...
plt.plot(SNR, mse_ecu, label = 'Soft decision-PM r =1',linestyle='-')
# plt.plot(SNR, mse_ecu_2, label = 'Soft decision-PM r =2',linestyle=':')
plt.plot(SNR, mse_ml, label = 'log-max',linestyle='--')
plt.plot(SNR, mse_ZF, label = 'Zero forcing',linestyle='-.')
plt.plot(SNR, mse_mmse, label = 'MMSE', linestyle='dashed', marker='o',)
plt.plot(SNR, mse, label = 'Exact')
