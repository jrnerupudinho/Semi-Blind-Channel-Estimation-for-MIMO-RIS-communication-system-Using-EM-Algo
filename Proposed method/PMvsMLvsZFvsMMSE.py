...
partition_r = 1
# #Computation 
mse_ml = np.zeros((monte_iter,len(T_p)))
mse_ZF = np.zeros((monte_iter,len(T_p)))
mse_mmse = np.zeros((monte_iter,len(T_p)))
# mse_ecu = np.zeros((monte_iter,len(T_p)))
mse_ecu_2 = np.zeros((monte_iter,len(T_p)))
...
for i in range(monte_iter):
    ...
    for tp in range(len(T_p)):      
      ...
      theta_hat_ec2 = em_pm(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,M_symbols,varn,itera,h_initial,h,n_tx,partition_r,X_d,qamCons)
      theta_hat_ml = em_ml(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial)
      theta_hat_ZF = em_zf(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h)
      theta_hat_mmse = em_mmse(Y_d,Y_p,T_d,T_p[tp],Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial,h)
      ...
      mse_ecu_2[i][tp] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ec2-h[:,np.newaxis]).T),(theta_hat_ec2-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      ...
      mse_ml[i][tp] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ml-h[:,np.newaxis]).T),(theta_hat_ml-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_ZF[i][tp] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_ZF-h[:,np.newaxis]).T),(theta_hat_ZF-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
      mse_mmse[i][tp] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat_mmse-h[:,np.newaxis]).T),(theta_hat_mmse-h[:,np.newaxis]))))/np.power(norm(h[:,np.newaxis]),2)
...
plt.plot(T_p, mse_ecu_2, label = 'Soft decision-PM r =1',linestyle=':')
plt.plot(T_p, mse_ml, label = 'log-max',linestyle='--')
plt.plot(T_p, mse_ZF, label = 'Zero forcing',linestyle='-.')
plt.plot(T_p, mse_mmse, label = 'MMSE', linestyle='dashed', marker='o',)
plt.plot(T_p, mse, label = 'Exact',linestyle='dashed', marker='x')
