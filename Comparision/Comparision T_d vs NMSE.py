...
#18th sept - the code is prefectly functional.
...
def channelMatrix1(varh,N,M,n_r,n_rx,n_tx):
  ...
  h_proposed = khatri_rao(H_BS.T, H_SU).flatten()
...
# def dataSymbols_approx(N,M,L_d,n_r,vard):
#   x_d = np.random.normal(loc=0, scale=np.sqrt(vard/2), size=(L_d, n_r*2)).view(np.complex128)
#   return x_d

# def pilotsSymbols_approx(N,M,L_p,n_r,vard):
#   x_p = np.random.normal(loc=0, scale=np.sqrt(vard/2), size=(L_p, n_r*2)).view(np.complex128)
#   return x_p

def dataSymbols_approx(L_d,M_symbols):
  qam = qp.QAModulation(M_symbols)
  qamCons = qam.constellation
  X_d = np.reshape(qamCons[np.random.choice(range(0, M_symbols), L_d, 'True')],(L_d,1))
  return X_d

def pilotsSymbols_approx(L_p,M_symbols):
  qam = qp.QAModulation(M_symbols)
  qamCons = qam.constellation
  X_p = np.reshape(qamCons[np.random.choice(range(0, M_symbols), L_p, 'True')],(L_p,1))
  return X_p
...
T_d = [20,30,40,50,60,70,80,90,100]; # Number of data symbols
L_d = T_d
T_p = 16; # Number of pilot symbols
L_p = T_p
...
# #Computation 
MSE_approx = np.zeros((monte_iter,len(L_d)))
MSE_proposed = np.zeros((monte_iter,len(T_d)))
for i in range(monte_iter):
    h,F_H,G,h_proposed = channelMatrix1(varh,N,M,n_r,n_rx,n_tx)
    X_p = pilotSymbols(n_tx,M_symbols,T_p)
    x_p = np.vstack(X_p)
    # x_p = pilotsSymbols_approx(L_p,M_symbols)
    for td in range(len(T_d)):      
      ...
      # x_d = dataSymbols_approx(L_d[td],M_symbols)
      Y_p,Y_d,Z_p,Z_d,h_initial = received_proposed(T_p,T_d[td],PsiTilde_tp,PsiTilde_td ,n_rx,n_tx,X_d,X_p,h_proposed,varn,M_symbols)
      z_p,z_d = noise_approx(varn,M,L_p,L_d[td])
      Y_p_dft,Y_d_dft = received_approx(h,F_H,PsiTilde_tp,PsiTilde_td,x_d,x_p,z_p,z_d)
      theta_hat = em_proposed(Y_d,Y_p,T_d[td],T_p,Z_p,PsiTilde_td ,all_possibleSymbols,M_symbols,varn,itera,h_initial)
      ...
      MSE_proposed[i][td] = np.matrix.trace(np.abs(np.matmul((np.conjugate(theta_hat-h_proposed[:,np.newaxis]).T),(theta_hat-h_proposed[:,np.newaxis]))))/np.power(norm(h_proposed[:,np.newaxis]),2)
      MSE_approx[i][td] = np.matrix.trace(np.abs(np.matmul((np.conjugate(G_cap_dft-G).T),(G_cap_dft-G))))/np.power(norm(G),2)
...
plt.plot(T_d, MSE_proposed, label = 'Proposed method')
plt.plot(T_d, MSE_approx, label = 'Approximation method')
